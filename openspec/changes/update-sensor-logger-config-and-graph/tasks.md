...
- [x] 8.1 RED: write `SamplingRateMappingTest` (pure JVM) asserting `Off.toSamplingPeriodUs() == null`, `Auto.toSamplingPeriodUs() == SensorManager.SENSOR_DELAY_NORMAL` (200_000), and `Hz(50).toSamplingPeriodUs() == 20_000`.
- [x] 8.2 RED: write `SamplingRateEncodingTest` asserting round-trip through `encode()` / `decode()`:
  - `Off ↔ "OFF"`
  - `Auto ↔ "AUTO"`
  - `Hz(50) ↔ "HZ:50"`
  - and asserting `decode("HZ:0")`, `decode("HZ:-3")`, `decode("HZ:abc")`, `decode("FOO")` all return `null` (caller substitutes per-sensor default).
- [x] 8.3 RED: extend `RecordingProfileStoreTest` to add:
...
- [x] 8.4 RED: add `LegacyDialogKeyTest` asserting `hasSeenDefaultsDialog()` emits `false` when only `seen_capture_defaults_dialog_v2 = true` exists in the store; the new code looks only at `_v3`.
- [x] 8.5 GREEN: replace `enum class SensorRateLevel` with `sealed class SamplingRate { object Off; object Auto; data class Hz(val value: Int) }` plus `fun toSamplingPeriodUs(): Int?`. Update `SensorCaptureSetting` field `rateLevel: SensorRateLevel → samplingRate: SamplingRate`.
- [x] 8.6 GREEN: implement `SamplingRate.encode(): String` and `fun SamplingRate.Companion.decode(s: String): SamplingRate?` accepting both new (`OFF` / `AUTO` / `HZ:<n>`) and legacy (`NORMAL` / `UI` / `GAME` / `FASTEST`) forms.
- [x] 8.7 GREEN: update `RecordingProfileStore` so reads call `decode(...)` per sensor (falling back to per-sensor default on `null`); writes always serialize via `encode(...)`.
- [x] 8.8 GREEN: bump bookkeeping key from `seen_capture_defaults_dialog_v2` to `seen_capture_defaults_dialog_v3`; leave the `_v2` key untouched (no migration write).
- [x] 8.9 GREEN: update `RecordingProfile.Default` to use `samplingRate = Auto` for the three pre-enabled sensors and `Off` for the rest.
- [x] 8.10 REFACTOR: collapse legacy-name mapping into a private `LEGACY_RATE_BY_NAME: Map<String, SamplingRate>` table referenced from `decode`.

**Acceptance criteria:**
- [x] 8.11 All 8.x tests pass.
- [x] 8.12 No references to `SensorRateLevel` or `rateLevel` remain in non-test production code.
- [x] 8.13 An on-device profile written by `v0.10.0-rc.1` loads without data loss into the new build.
...
- [x] 9.1 RED: update `planRegistrations`-based tests so a profile with gyroscope `samplingRate = Hz(50)` plans a registration whose `samplingPeriodUs` is `20_000`.
- [x] 9.2 RED: assert that `samplingRate = Off` (with any combination of `enabled` / `visibleOnGraph`) produces no plan entry for that sensor.
- [x] 9.3 RED: assert `samplingRate = Auto` plans `samplingPeriodUs = SensorManager.SENSOR_DELAY_NORMAL`.
- [x] 9.4 GREEN: refactor `shouldRegister(setting)` to `setting.samplingRate != Off && (setting.enabled || setting.visibleOnGraph)`.
- [x] 9.5 GREEN: refactor `planRegistrations(profile, probe)` to return `(sensor, samplingPeriodUs: Int)` pairs derived via `setting.samplingRate.toSamplingPeriodUs()`.
- [x] 9.6 GREEN: update `SensorRecordingService.registerSensorsFromProfile` to call `sensorManager.registerListener(this, sensor, samplingPeriodUs, MAX_REPORT_LATENCY_US)`.
- [x] 9.7 REFACTOR: drop the now-unused `Int?`-returning enum helper.

**Acceptance criteria:**
- [x] 9.8 All 9.x tests pass.
- [x] 9.9 No call site passes a hardware-relative `SensorManager.SENSOR_DELAY_*` constant except the one inside `SamplingRate.Auto.toSamplingPeriodUs()`.
...
- [x] 10.1 RED: extend `SensorCaptureConfigState` test suite to cover:
...
- [x] 10.2 RED: extend `ConfigRowStateTest` so the Unavailable row asserts disabled state across the new three controls (Enable checkbox, Show-on-graph checkbox, rate selector with Custom Hz input).
- [x] 10.3 GREEN: update the row layout: Enable checkbox + indented Show-on-graph checkbox + rate selector with three options (`Off`, `Auto`, `Custom`); when `Custom` is selected, reveal an integer input field bound to the row's working-copy `Hz` value.
- [x] 10.4 GREEN: `SensorCaptureConfigViewModel` (and its headless `State`):
...
- [x] 10.5 GREEN: remove the "Normal / UI / Game / Fastest" labels from the layout XML and string resources.
- [x] 10.6 GREEN: update the absent-sensor row's `buildRowState` to disable the new controls (Enable, Show-on-graph, the rate selector, and the Custom Hz input).
- [x] 10.7 REFACTOR: rate-selector logic isolated in the headless `SensorCaptureConfigState` (`RateMode` + `RateDraft`); the adapter's `bind(...)` only renders/relays it. (A separate `RateSelectorBinder` class was unnecessary given the logic already lives in the testable state layer.)

**Acceptance criteria:**
- [x] 10.8 All 10.x tests pass.
- [x] 10.9 Enabling a previously-off sensor results in `samplingRate = Auto` without a second user interaction.
- [x] 10.10 Custom Hz selection persists across screen reopen and across app restart (covered transitively by ticket 8's round-trip).
...
- [x] 11.1 RED: write `LiveGraphRateLabelTest` covering `computeRollingHz(samples)` — pure JVM:
...
- [x] 11.2 RED: write `LiveGraphLabelFormatterTest` asserting `formatRateLabel(null) == "—"` and `formatRateLabel(48.7) == "~49 Hz"`.
- [x] 11.3 GREEN: implement `computeRollingHz(samples: List<SensorSample>): Double?` and `formatRateLabel(hz: Double?): String` in the same package as `LiveGraphView`.
- [x] 11.4 GREEN: in `LiveGraphView`, when drawing each band, render the sensor name and the formatted rate label (e.g. `"Accelerometer  ~48 Hz"`) at the band's top-left. No additional canvas operations on bands with fewer than 2 samples.
- [x] 11.5 REFACTOR: hoist label typeface/size to a `companion object` constant.

**Acceptance criteria:**
- [x] 11.6 All 11.x tests pass.
- [x] 11.7 The rate label updates at the same cadence as the rest of the band (≤20 Hz, gated by `LIVE_STREAM_COALESCE_MS`).
...
- [x] 12.1 Run `openspec validate update-sensor-logger-config-and-graph --strict` and resolve issues.
- [x] 12.2 `grep -rn "SensorRateLevel\\|rateLevel" android/app/src/main/` returns zero non-test hits.
- [x] 12.3 Run `just test` (Docker) — all unit tests pass.
- [x] 12.4 Run `just build` (Docker) — debug APK builds cleanly.
...
**Acceptance criteria:**
- [x] 12.8 `openspec validate` passes strict on the amended change.
- [x] 12.9 `just test` passes.
- [x] 12.10 `just build` passes.
